...
template< typename Type, size_t Capacity
        , typename LogStrategy = NullLogger
        , typename ErrorStrategy = Abort >
   ...
   : private LogStrategy
{
 ...
 private:
   ...
   using LogStrategy::log_error;

   void handle_error( std::string_view message ) const
   {
      ErrorStrategy error{};
      error( message );
   }
...
int main()
{
   FixedVector<int,20,ConsoleLogger,Exception> v{ 1, 2, 3, 4, 5 };
