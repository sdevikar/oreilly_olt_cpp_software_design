...
class Shape
{
 public:
   ...
   virtual std::string serialize( /*Serialization-related parameters*/ ) const = 0;
};
...
template< typename DrawStrategy, typename SerializationStrategy >
class Circle : public Shape
{
 public:
   Circle( double radius, DrawStrategy drawer, SerializationStrategy serializer )
      : radius_{ radius }
      , center_{}
...
{
 public:
   Square( double side, DrawStrategy drawer, SerializationStrategy serializer )
      : side_{ side }
      , center_{}
      , drawer_{ std::move(drawer) }
      , serializer_{ std::move(serializer) }
...
class GLDrawer
{
 public:
   ...
   void operator()( Circle<GLDrawer,SerializationStrategy> const& circle ) const
   {
   ...
   template< typename SerializationStrategy >
   void operator()( Square<GLDrawer,SerializationStrategy> const& square ) const
   {
...
class FSSerializer
{
 public:
   ...
   std::string operator()( Circle<DrawStrategy,FSSerializer> const& circle ) const
   {
      fs::Serializer serializer{};
      serializer << typeid(Circle<DrawStrategy,FSSerializer>).hash_code()
                 << circle.radius() << circle.center().x << circle.center().y;
      return serializer.to_string();
   }

   template< typename DrawStrategy >
   std::string operator()( Square<DrawStrategy,FSSerializer> const& square ) const
   {
      fs::Serializer serializer{};
      serializer << typeid(Square<DrawStrategy,FSSerializer>).hash_code()
...
int main()
{
   ...
   shapes.emplace_back(
      std::make_unique<Circle<GLDrawer,FSSerializer>>( 2.3, GLDrawer{gl::Color::red}, FSSerializer{} ) );
   shapes.emplace_back(
      std::make_unique<Square<GLDrawer,FSSerializer>>( 1.2, GLDrawer{gl::Color::green}, FSSerializer{} ) );
   shapes.emplace_back(
      std::make_unique<Circle<GLDrawer,FSSerializer>>( 4.1, GLDrawer{gl::Color::blue}, FSSerializer{} ) );
