...
enum class ShapeType
{
   circle,
   square,
   triangle  // Intrusive (breaking) change!
};
...
class Triangle : public Shape  // Intrusive addition on the "wrong" side of the architecture!
{
 public:
...
void draw( Triangle const& triangle, gl::Color color )
{
   std::cout << "triangle: side=" << triangle.side()
             << ", color = " << gl::to_string(color) << '\n';
}
   ...
   for( auto const& shape : shapes )
   {
      switch( shape->getType() )
      {
         ...
         case ShapeType::triangle:
            draw( *static_cast<Triangle const*>( shape.get() ), gl::Color::blue );
            break;
      }
   }
}
...
void serialize( Circle const& circle, fs::Serializer& serializer )
{
   serializer << typeid(Circle).hash_code() << circle.radius()
              << circle.center().x << circle.center().y;
}

void serialize( Square const& square, fs::Serializer& serializer )
{
   serializer << typeid(Square).hash_code() << square.side()
              << square.center().x << square.center().y;
}

void serialize( Triangle const& square, fs::Serializer& serializer )
{
   serializer << typeid(Triangle).hash_code() << square.side()
              << square.center().x << square.center().y;
}
...
void serializeAllShapes( Shapes const& shapes )
{
   ...
   for( auto const& shape : shapes )
   {
      ...
      {
         case ShapeType::circle:
            serialize( *static_cast<Circle const*>( shape.get() ), serializer );
            break;
         case ShapeType::square:
            serialize( *static_cast<Square const*>( shape.get() ), serializer );
            break;
         case ShapeType::triangle:
            serialize( *static_cast<Triangle const*>( shape.get() ), serializer );
            break;
      }
   }
