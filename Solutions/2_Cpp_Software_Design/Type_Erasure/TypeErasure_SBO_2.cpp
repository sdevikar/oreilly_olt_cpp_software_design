...
class Shape
{
 public:
        ...
        , SerializationStrategy const& serializer )
   {
      ...
      pimpl_ = ::new (buffer.data()) M( shape, drawer, serializer );  // Placement new or C++20 'std::construct_at()'
   }

   ~Shape() { pimpl_->~Concept(); }  // Or C++17 'std::destroy_at()'

   Shape( Shape const& other )
   {
      pimpl_ = other.pimpl_->clone( buffer.data() );
   }
 ...
 private:
   ...
   struct Concept
   {
      ...
      virtual Concept* clone( void* memory ) const = 0;
   };
   ...
   struct Model final : public Concept
   {
      ...
      Concept* clone( void* memory ) const final { return ::new (memory) Model(*this); }  // Or C++20 'std::construct_at()'
   ...
   Concept* pimpl_;  // Stores a pointer to the Model created in the constructor.
                     // Alternatively, it is possible to use a reinterpret_cast (see
