...
class Model10
{
 public:
   ...
   Model10( Model10 const& other );
   Model10& operator=( Model10 const& other );

   Model10( Model10&& other ) noexcept;
   Model10& operator=( Model10&& other ) noexcept;

   void drive();
   // ... more car-specific functions

 private:
   constexpr static size_t capacity  = 16UL;
...
Model10::Model10( Model10 const& other )
{
   construct_at( pimpl(), *other.pimpl() );  // Or C++20 'std::construct_at()'
}

Model10& Model10::operator=( Model10 const& other )
{
   *pimpl() = *other.pimpl();
   return *this;
}

Model10::Model10( Model10&& other ) noexcept
{
   construct_at( pimpl(), std::move(*other.pimpl()) );  // Or C++20 'std::construct_at()'
}

Model10& Model10::operator=( Model10&& other ) noexcept
{
